...
from Bio.PDB.Polypeptide import protein_letters_3to1_extended # updated
...
class LigandPocketDDPM(pl.LightningModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == 'fit': # means during training
    ...
    def get_ligand_and_pocket(self, data):
        ...
        if self.virtual_nodes:
            ligand['num_virtual_atoms'] = data['num_virtual_atoms'].to(self.device, INT_TYPE)
        ...
        with torch.no_grad():
            if 'one_hot' in pocket and pocket['one_hot'].shape[1] == 11 and self.ddpm.atom_nf == 10:
                pocket['one_hot'] = pocket['one_hot'][:, :-1]  # temp fix
                
        return ligand, pocket, names
    
    def forward(self, data):
        ligand, pocket, _ = self.get_ligand_and_pocket(data)
    ...
    # switch back to original once i have added the validation step
    def validation_step(self, batch, batch_idx):
        if self.trainer.is_global_zero and batch_idx == 0:
            print("Validation step placeholder")
            return {'val_placeholder': 0}
    ...
    def on_validation_epoch_end(self):
        ...
        print(f"[Rank {self.global_rank}] Starting on_validation_epoch_end at epoch {self.current_epoch}")
        
        suffix = '' if self.mode == 'joint' else '_given_pocket'

        # Evaluate samples and log metrics
        if (self.current_epoch + 1) % self.eval_epochs == 0:
            try:
                sampling_results = getattr(self, 'sample_and_analyze' + suffix)(
                    n_samples=self.eval_params.n_eval_samples,
                    dataset=self.val_dataset,
                    batch_size=self.eval_batch_size
                )
                
                print(f"Calculated metrics: {sampling_results}")
                
                self.log_metrics(sampling_results, 'val', batch_size=self.eval_batch_size)
                
            except Exception as e:
                print(f"Error in sample_and_analyze: {e}")
                import traceback
                traceback.print_exc()
            finally:
                # Clean up memory
            ...
            print(f"[Rank {self.global_rank}] Skipping sample_and_analyze{suffix}: epoch condition not met")
        ...
        if (self.current_epoch + 1) % self.visualize_sample_epoch == 0:
            ...
            try:
                ...
                print(f'[Rank {self.global_rank}] Sample visualization took {time() - tic:.2f} seconds')
            except Exception as e:
        ...
        # Visualize denoising chain - only if enabled
        if self.visualize_chain_epoch > 0 and (self.current_epoch + 1) % self.visualize_chain_epoch == 0:
            tic = time()
            try:
                getattr(self, 'sample_chain_and_save' + suffix)(
                    self.eval_params.keep_frames
                )
                print(f'[Rank {self.global_rank}] Chain visualization took {time() - tic:.2f} seconds')
            except Exception as e:
                print(f"Error in chain visualization: {e}")
            finally:
                # Clean up memory
                gc.collect()
                torch.cuda.empty_cache()
        ...
        # This is crucial - reset the should_stop flag explicitly
        if hasattr(self.trainer, "should_stop"):
            self.trainer.should_stop = False
            
        print(f"[Rank {self.global_rank}] Completed on_validation_epoch_end at epoch {self.current_epoch}")
    ...
    def prepare_pocket(self, biopython_residues, repeats=1):
        ...
        if self.pocket_representation == 'CA':
            ...
            pocket_types = torch.tensor(
                [self.pocket_type_encoder[protein_letters_3to1_extended(res.get_resname())]
    ...
    def generate_ligands(self, pdb_file, n_samples, pocket_ids=None,
                         ...
                         n_nodes_bias=0, n_nodes_min=0, save_traj=False, **kwargs):
        """
        ...
        if type(self.ddpm) == EnVariationalDiffusion:
            ...
            xh_lig, xh_pocket, lig_mask, pocket_mask = self.ddpm.inpaint(
                ...
                timesteps=timesteps, return_frames=frames, **kwargs)

        # Use conditional generation
        elif type(self.ddpm) == ConditionalDDPM:
            xh_lig, xh_pocket, lig_mask, pocket_mask, _, _ = \
                self.ddpm.sample_given_pocket(pocket, num_nodes_lig, 
                                              timesteps=timesteps, return_frames=frames)
        else:
            raise NotImplementedError
        
        # ADDED: Process intermediate states as molecules for the trajectory
        if save_traj:
            xh_lig = utils.reverse_tensor(xh_lig)
            xh_pocket = utils.reverse_tensor(xh_pocket)

            lig_mask = torch.arange(xh_lig.size(0), device=self.device
                                    ).repeat_interleave(len(lig_mask))
            pocket_mask = torch.arange(xh_pocket.size(0), device=self.device
