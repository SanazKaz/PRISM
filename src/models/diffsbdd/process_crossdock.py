...
from Bio.PDB.Polypeptide import protein_letters_3to1_extended, is_aa # three_to_one deprecated.
                ...
                for atom in res.get_atoms():
                    if atom.name == 'CA':
                        pocket_one_hot.append(np.eye(1, len(amino_acid_dict),
                                                     amino_acid_dict[protein_letters_3to1_extended[(res.get_resname())]]).squeeze()) # may be wrong 
...
def compute_smiles(positions, one_hot, mask):
    ...
    for i, (pos, atom_type) in pbar:
        print(f"Starting to process molecule {i}")
        mol = build_molecule(pos, atom_type, dataset_info)
        
        if mol is None:
            print(f"OpenBabel failed for molecule {i} - build_molecule returned None")
            continue
        ...
        try:
            ...
            print(f"Sanitization failed for molecule {i}: {str(e)}")
            continue
        ...
        if mol is not None:
            ...
            print(f"rdmol_to_smiles failed for molecule {i} - returned None")
...
if __name__ == '__main__':
    ...
    for split in data_split.keys():
        ...
        for pocket_fn, ligand_fn in pbar:
            ...
            # first try: PDB parsing
            
            try:
                struct_copy = PDBParser(QUIET=True).get_structure('', pdbfile)
            except Exception as e:
                num_failed += 1
                failed_save.append((pocket_fn, ligand_fn, f"PDB PARSING ERROR:{str(e)}")) # catch PDB parse error
                print(f"PDB parsing failed for {pocket_fn}: {str(e)}")
                pbar.set_description(f'#failed: {num_failed}')


                continue
            # second try: ligand and pocket processing
            try:
                ligand_data, pocket_data = process_ligand_and_pocket(
                    ...
                    error_msg = str(e)
                    num_failed += 1
                    failed_save.append((pocket_fn, ligand_fn, f"{error_type}: {error_msg}"))
                    pbar.set_description(f'#failed: {num_failed}')
                    continue
            
            # third try: data appending issue (unlikely)
            ...
            try:
                ...
                num_failed += 1
                failed_save.append((pocket_fn, ligand_fn, f"Data appending error: {str(e)}"))
                print(f"Data appending failed for {pocket_fn}_{ligand_fn}: {str(e)}")
