...
class ConditionalDDPM(EnVariationalDiffusion):
    ...
    def kl_prior(self, xh_lig, mask_lig, num_nodes):
        ...
        # Compute the last alpha value, alphåa_T.
    ...
    def noised_representation(self, xh_lig, xh0_pocket, lig_mask, pocket_mask,
        ...
        # print(f"DEBUG - noised_representation input shapes:")
        # print(f"  xh_lig shape: {xh_lig.shape}")
        # print(f"  n_dims: {self.n_dims}, atom_nf: {self.atom_nf}")
        # print(f"  Expected eps_lig shape: {(len(lig_mask), self.n_dims + self.atom_nf)}")
        # print(f"  xh_lig.shape[1]: {xh_lig.shape[1]}")
        
        # Compute alpha_t and sigma_t from gamma.
        alpha_t = self.alpha(gamma_t, xh_lig) # scaling factor at time t
        sigma_t = self.sigma(gamma_t, xh_lig) # noise level at time t

        # Sample zt ~ Normal(alpha_t x, sigma_t)
        eps_lig = self.sample_gaussian(
            size=(len(lig_mask), xh_lig.shape[1]),
            device=lig_mask.device)
        
        # print(f"  Sampled eps_lig shape: {eps_lig.shape}")
    ...
    def forward(self, ligand, pocket, return_info=False):
        ...
        # print(f"Forward pass: n_dims={self.n_dims}, atom_nf={self.atom_nf}")
        # print(f"xh0_lig shape: {xh0_lig.shape}")
    ...
    def get_log_probs(self, molecule_data):
        """
        Calculate log probabilities of molecules given by the policy network.
        
        Args:
            molecule_data: Can be a tuple of (xh_lig, xh_pocket) or a more complex structure
        
        Returns:
            Tensor of log probabilities, one for each molecule in the batch
        """
        # Handle different data structures that could be passed
        if isinstance(molecule_data, tuple):
            if len(molecule_data) == 2:
                # Handle (xh_lig, xh_pocket) format
                xh_lig, xh_pocket = molecule_data
                # Create batch indices since they weren't provided
                batch_size = xh_lig.shape[0]
                lig_mask = torch.arange(batch_size, device=xh_lig.device)
                pocket_mask = torch.arange(batch_size, device=xh_pocket.device)
            elif len(molecule_data) == 4:
                # Handle (xh_lig, xh_pocket, lig_mask, pocket_mask) format
                xh_lig, xh_pocket, lig_mask, pocket_mask = molecule_data
            else:
                raise ValueError(f"Unexpected tuple length: {len(molecule_data)}")
        else:
            raise ValueError(f"Unexpected molecule_data type: {type(molecule_data)}")
        
        # Create timestep tensor (zeros for final step prediction)
        t = torch.zeros(size=(xh_lig.shape[0], 1), device=xh_lig.device)
        
        # Call dynamics to get predictions
        net_out_lig, _ = self.dynamics(
            xh_lig, xh_pocket, t, lig_mask, pocket_mask
        )
        
        # Compute log probability per molecule
        log_probs = -0.5 * torch.sum(net_out_lig**2, dim=1)
    ...
    def log_p_zs_given_zt(self, s, t, z_t, z_s, xh_pock, lig_mask, pock_mask):
        """
        Computes log p(z_s | z_t) for a given transition.
        This is called by the PPO trainer during the update step.
        It performs a forward pass with the CURRENT policy weights to get a
        fresh epsilon prediction.
        """
        ...
        # 1. Get a FRESH epsilon prediction using the CURRENT model weights.
        # This is the on-policy evaluation step.
        eps_t_lig, _ = self.dynamics(
            z_t, xh_pock, t, lig_mask, pock_mask
        )

        # 2. Compute the mean of the distribution that WOULD have been sampled.
        ...
        # 4. Calculate the log probability of the ORIGINAL next_state (z_s)
        # under this NEW distribution.
        sigma_sq_lig = sigma[lig_mask]**2 + 1e-8
        diff = z_s - mu_lig  # Note: we use z_s from the rollout buffer
        
        log_prob_atom = -0.5 * (
            (diff**2 / sigma_sq_lig).sum(dim=1) + 
            torch.log(2 * torch.pi * sigma_sq_lig).sum(dim=1)
        )
    ...
    def sample_p_zs_given_zt(self, s, t, zt_lig, xh0_pocket, ligand_mask, pocket_mask, fix_noise=False):
        """
        Samples from zs ~ p(zs | zt) and returns the sample along with its correctly calculated log probability.
        This version calls dynamics ONCE to prevent drift.
        this is identical to log_p_zs_given_zt
        """
        gamma_s = self.gamma(s)
        gamma_t = self.gamma(t)
        ...
        # 1. Call dynamics only ONCE to get the predicted noise (the "action").
        eps_t_lig, _ = self.dynamics(
            zt_lig, xh0_pocket, t, ligand_mask, pocket_mask
        )

        # 2. Compute the mean of the distribution using the single prediction.
        mu_lig = zt_lig / alpha_t_given_s[ligand_mask] - \
                (sigma2_t_given_s / alpha_t_given_s / sigma_t)[ligand_mask] * eps_t_lig

        # 3. Compute the standard deviation of the distribution.
        sigma = sigma_t_given_s * sigma_s / sigma_t

        # 4. Sample the next state, zs, from the distribution Normal(mu, sigma).
        zs_lig, xh0_pocket = self.sample_normal_zero_com(
            mu_lig, xh0_pocket, sigma, ligand_mask, pocket_mask, fix_noise
        )
        
        # 5. Correctly compute the log probability of the sample we just drew.
        # We add a small epsilon for numerical stability.
        sigma_sq_lig = sigma[ligand_mask]**2 + 1e-8
        diff = zs_lig - mu_lig
        
        # log prob for each atom
        log_prob_atom = -0.5 * (
            (diff**2 / sigma_sq_lig).sum(dim=1) + 
            torch.log(2 * torch.pi * sigma_sq_lig).sum(dim=1)
        )
        
        # Sum log probs over atoms to get log prob per molecule
        log_prob = scatter_mean(log_prob_atom, ligand_mask, dim=0)

        self.assert_mean_zero_with_mask(zt_lig[:, :self.n_dims], ligand_mask)

        # IMPORTANT: Return the calculated log_prob here. The sample_given_pocket will now receive it.
        return zs_lig, xh0_pocket, log_prob
    ...
    @torch.no_grad() # has to stay here 
    def sample_given_pocket(self, pocket, num_nodes_lig, return_frames=1, timesteps=None):
        ...
        lig_mask = utils.num_nodes_to_batch_mask(n_samples, num_nodes_lig, device=device)
        ...
        z_lig, xh_pocket = self.sample_normal_zero_com(mu_lig, xh0_pocket, sigma, lig_mask, pocket['mask'])
        ...
        for s in reversed(range(0, timesteps)):
            ...
            s_array = s_array / timesteps
            t_array = t_array / timesteps
            # print(f"[DEBUG] Step {s}: s_array={s_array.shape}, t_array={t_array.shape}")
            
            z_lig, xh_pocket, step_log_prob = self.sample_p_zs_given_zt(
                s_array, t_array, z_lig, xh_pocket, lig_mask, pocket['mask']
            )
            
            log_probs.append(step_log_prob)
            z_states.append(z_lig.clone())
        ...
        return out_lig.squeeze(0), out_pocket.squeeze(0), lig_mask, pocket['mask'], log_probs, z_states
