...
class EGNNDynamics(nn.Module):
    ...
    def forward(self, xh_atoms, xh_residues, t, mask_atoms, mask_residues):
        ...
        if torch.any(torch.isnan(vel)):
                print("NaN detected in EGNN output!")
                print(f"Input xh_atoms: {xh_atoms}")
                print(f"Input xh_residues: {xh_residues}")
                print(f"Timestep t: {t}")
                print(f"mask_atoms: {mask_atoms}")
                print(f"mask_residues: {mask_residues}")
                print(f"vel: {vel}")
        ...
        output_lig = torch.cat([vel[:len(mask_atoms)], h_final_atoms], dim=-1)
        output_res = torch.cat([vel[len(mask_atoms):], h_final_residues], dim=-1)
