...
class PoseBustersGeometryReward(BaseReward):
    ...
    def score_mol(self, mol: Mol) -> float:
        ...
        # Fixed per-term penalty weights (previously bond 1.0, angle 0.5,
        # clash 1.5; retuned to emphasise angle violations).
        W_BOND, W_ANGLE, W_CLASH = 1.5, 5.0, 1.0
        ...
        for _, row in df_clashes.iterrows():
            ...
            if clash_pen < -self.threshold_clash:
                total_penalty += abs(clash_pen) * W_CLASH
