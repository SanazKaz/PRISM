...
# TargetDiff pretrained checkpoint (native .pt format, key 'model')
...
# --- 2. LOGGING SETUP ---
JOB_NAME="targetdiff_penlogp_CD"
SLURM_LOG_DIR="${PROJECT_ROOT}/logs/${JOB_NAME}/${DATASET_NAME}"
...
echo "Warm start ckpt:   $WARM_START_CKPT"
...
# --- 3. STAGE IN (copy dataset to scratch) ---
...
# --- 5. RUN TRAINING ---
echo "Starting TargetDiff PPO training..."
srun python "${PROJECT_ROOT}/scripts/train.py" \
    --config "${PROJECT_ROOT}/configs/targetdiff_ppo.yaml" \
