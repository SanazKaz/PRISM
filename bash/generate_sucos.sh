...
#SBATCH --job-name=diffsbdd_model_results_plus
#SBATCH --mail-user=wolf7055@ox.ac.uk
#SBATCH --output=jobs_files/diffsbdd_model_results_plus.log
...
for outer_idx in $(seq 1 $outer_batch_size); do
    file_number=$((outer_idx))
    
    echo "Generating outer batch $file_number of 150..."
    for inner_idx in $(seq 1 $inner_batch_size); do
        ...
        python src/models/diffsbdd/generate_ligands.py \
            ...
            --outfile results/qed/qed_model_results/7e2z_qed_model_${file_number}_inner${inner_idx}_gen_plus.sdf \
