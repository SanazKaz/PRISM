...
EXCLUDE_LIST = {
    ...
    'BCN', 'AZI', 'SCN', 'CYN', 'OCT', 'UNL', 'UNX', 'CLR', 'J40'
}
...
def extract_pocket_biopython(pdb_path, sdf_path, distance_cutoff, output_pdb_path):
    """
    Extracts and saves a pocket PDB using Biopython and RDKit.
    """
    # 1. Load Ligand and get coordinates
    try:
        # Use MolFromMolFile for simplicity, robust_read_sdf could also be used
    ...
    try:
        ...
        raise Exception(f"Biopython failed to parse PDB {pdb_path.name}: {e}")

    # 3. Find pocket residues
    pocket_residues = []
    for residue in structure.get_residues():
        ...
        try:
            ...
            print(f"    [WARN] Could not get coords for residue {residue.id} in {pdb_path.name}")
            continue
        ...
        # 5. Keep residue if it's close enough
        if min_dist < distance_cutoff:
            pocket_residues.append(residue)

    # 6. Save the new pocket PDB
...
def create_binding_pockets(args):
    ...
    print(f"Output split file will be saved to: {split_file_path.resolve()}")
    ...
    for pdb_path in pdb_files:
        ...
        try:
            # Get all non-polymer entity IDs for the entry
            entry_url = f"https://data.rcsb.org/rest/v1/core/entry/{pdb_id}/"
            response = requests.get(entry_url)
            response.raise_for_status()
            data = response.json()
            entity_ids = data["rcsb_entry_container_identifiers"].get("non_polymer_entity_ids")

            if not entity_ids:
                print("  No non-polymer entities found. Skipping.")
                continue

            # Loop over each entity to find its "true" ligand instances
            for entity_id in entity_ids:
                # Get entity details (to find 3-letter comp_id)
                ...
                # Get all chain instances for this ligand
                ...
                for chain in asym_ids:
                    try:
                        # Get instance details (to find auth_seq_id)
                        ...
                        # Define the unique, shared base name
                        ...
                        print(f"    [OK] Saved Ligand: {sdf_path.name}")

                        # --- 6. MODIFIED: Biopython Pocket Extraction ---
                        # This block replaces the entire PyMOL section
                        ...
                        print(f"    [OK] Saved Pocket: {output_pdb_path.name}")
            ...
            print(f"  [FAIL] Failed to process PDB {pdb_id}: {e}", file=sys.stderr)
        ...
        try:
            with open(split_file_path, 'w') as f:
                for name in successful_basenames:
                    f.write(name + '\n')
            print(f"\nSuccessfully generated split file with {len(successful_basenames)} entries:")
            print(f"  {split_file_path.resolve()}")
        except Exception as e:
            print(f"    [FAIL] Error saving split file: {e}", file=sys.stderr)
    else:
        print("\nNo successful basenames were found. No split file was generated.")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str, 
        required=False, 
        default=None, # Handled in main function to set default './funfam_data'
        help="Main directory to save all outputs (pockets, SDFs, split file). "
             "If not given, a default './funfam_data' directory will be created."
    )
